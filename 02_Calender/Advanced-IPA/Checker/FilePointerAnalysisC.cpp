...
bool Checker::Checker_FilePointerAnalysisC(wBasicBlock *BB, wInstruction *Inst){
	...
	if(Inst->getInstruction()->getOpcodeName() == std::string("call")){		
		...
		/* if free is found */
		if(get_opnd->getName().str() == std::string("fclose")){
#ifdef DEBUG_CHECKER				
			std::cout << "[First Free]: ";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(Inst->getInstruction()->getOpcodeName() == std::string("call")){		
		...
		Value *get_opnd = bare_inst->getOperand(bare_inst->getNumOperands() - 1);
		/* if malloc is found */
		if(get_opnd->getName().str() == std::string("fopen")){
